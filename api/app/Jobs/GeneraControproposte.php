...
private function creaPromptOpenAI($preventivoPaziente, $listinoMedico)
{
    return [
        [
            'role' => 'system',
            'content' => 'Sei un assistente per uno studio dentistico. Il tuo compito è creare una controproposta per un paziente basandoti sul suo preventivo e sul listino prezzi dello studio. Analizza semanticamente ogni "prestazione" nel preventivo del paziente e trova la corrispondenza migliore nel "listino_medico". Il JSON di output deve avere una chiave "voci_proposta" (un array di oggetti) e una chiave "totale_proposta" (un numero). Per ogni voce del preventivo del paziente, crea un oggetto nell\'array "voci_proposta" con le chiavi: "prestazione_originale" (dal preventivo del paziente), "prestazione_corrispondente" (la voce trovata nel listino medico, o un messaggio che indica nessuna corrispondenza), e "prezzo" (il prezzo dal listino medico). Se non trovi una corrispondenza esatta o semanticamente molto simile, imposta il prezzo a 0. Calcola la somma di tutti i prezzi e inseriscila in "totale_proposta".'
        ],
        [
            'role' => 'user',
